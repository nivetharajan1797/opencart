@lessonVideoAndVtt @regression
Feature: To create lesson with Video and Vtt
  To test Lesson having video with Vtt, and verifying lesson content as student.
  ...
  Scenario: Verify course outline for Lesson
    ...
    And user enter video and vtt details on lesson page
      | Title        | videoID  		| vttUrl  | vttLanguage |
      | lesson.title | vtt.video.id | vtt.url | English     |
    ...
    And user verifies the lesson video and vtt text
      | Title        | videoID  | vttText          |
      | lesson.title | video.id | vttTextToVerify  |