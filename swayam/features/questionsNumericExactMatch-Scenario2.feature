@questionsNumericExactMatchType @questionsNumericExactMatchType2 @regression_set2 
Feature: Questions - Numeric - Exact match Type - scenario 2
...
Scenario: Verify creation of numeric assessment with exact value, student submission and scores 
	...
	And user enter following required details on assessment page 
		...
		| assessment.numeric.desc2		|      5 | No                  | No                | future   |
	...
	Then user adds numeric exact match question 
		...
		| assessment.numeric.question2	|	assessment.numeric.answer2	|	assessment.numeric.desc2	|
	And user clicks on "Save" Link 
	...
	And user set "assessment.numeric.desc2" availability to "Course" 
	And user clicks on "Save" Link 
	And user waits for "Saved" confirmation 
	...
	And user goes to assessment "assessment.numeric.desc2" on course outline 
	And store page url to "assessment.user.page" 
	And user attempts numeric type question 
		|	Answer						|
		| assessment.numeric.answer2	|
	And user submits the assessment 
	And user verifies the confirmation message "submit.confirmation.msg" for "assessment.numeric.desc2" 
	...
	And user verifies the score on progress page 
		| entityname                	| score |
		| assessment.numeric.desc2	| 100.0 |
	And user logout from user side 
	And user clears the browser cache 
